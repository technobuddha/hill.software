...
import type { PartialColor } from '../src/color';

const units: [ string, PartialColor ][] = [
...
describe(
    ...
    () => {
        test.each(units)(
            '%s',
            (src, dst) => {
                expect(color.parse(src)).toMatchObject(dst);
            }
        );
        ...
        test(
            'invalid format',
            () => {
                expect(() => color.parse('foobie-bletch')).toThrow('Color can not be parsed');
            }
        );
    }
);
