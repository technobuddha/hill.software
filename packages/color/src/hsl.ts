...
export namespace hsl {
    ...
    type iHSL = { hue: number; saturation: number; lightness: number; alpha?: number };

    export function internal(color: HSL): iHSL {
        return {
    ...
    export function external({ hue, saturation, lightness, alpha }: iHSL): HSL {
        return {
            h: round(hue        * 360, 0),
    ...
    export function parse(input: string): HSL | undefined {
        const rgb   = re`^hsla?${op}${angle}${sep}${percent}${sep}${percent}${alpha}${cp}$`;
